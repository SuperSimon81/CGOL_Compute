...
static int32_t runApiThread(bx::Thread *self, void *userData)
{
	...
	bgfx::TextureHandle A_texture = bgfx::createTexture2D(
		...
		BGFX_TEXTURE_COMPUTE_WRITE|BGFX_SAMPLER_NONE|BGFX_TEXTURE_BLIT_DST);
	...
	bgfx::TextureHandle B_texture = bgfx::createTexture2D(
		...
		bgfx::TextureFormat::RGBA32F,
		BGFX_TEXTURE_COMPUTE_WRITE|BGFX_SAMPLER_NONE|BGFX_TEXTURE_BLIT_DST);
	bgfx::UniformHandle delta_time_uniform = bgfx::createUniform("dt", bgfx::UniformType::Vec4);
	...
	while (!exit)
	{
		...
		//{
			...
			bgfx::dbgTextPrintf(0, 0, 0x0f, "FPS: %d and %f",frameCount,currentTime - previousTime);
			frameCount = 0;
		...
		bgfx::setUniform(delta_time_uniform,value_ptr(vec4((currentTime - previousTime),currentTime,0,0)));
